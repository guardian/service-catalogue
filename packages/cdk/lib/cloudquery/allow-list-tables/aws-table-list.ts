/**
 * An auto-generated list of all possible tables collected by CloudQuery.
 * This list is obtained via the CloudQuery CLI.
 *
 * Given the YAML file (aws.yaml):
 *
 * ```yaml
 * kind: source
 * spec:
 ...
 *
 * # Copy table list
 * cat cq-docs/aws/__tables.json | jq '[.[] | .name] + [.[] | .relations[].name] | .[]' | sort | uniq | pbcopy
 * ```
 *
 * TODO - automate this process to make it easier to update CloudQuery plugins
 *
 * @see https://docs.cloudquery.io/docs/cli-reference/cloudquery_tables
 */
const availableAwsTables: string[] = [
...
export const awsTables = availableAwsTables.filter(
	(table) => !skippedAwsTables.includes(table),
);
