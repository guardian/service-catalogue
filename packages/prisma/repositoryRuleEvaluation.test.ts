...
import { GitHubRepository, GitHubRepositoryBranch } from './model';

const thePerfectRepo: GitHubRepository = {
	full_name: 'repo1',
	default_branch: 'main',
	topics: ['production'],
	id: BigInt(1),
...
describe('Repositories should have branch protection', () => {
	test('We should get an affirmative result when the default branch is protected', () => {
		...
		const protectedMainBranch: GitHubRepositoryBranch = {
			repository_id: BigInt(1),
				name: 'main',
			protected: true,
			protection: {},
		}
		const unprotectedSideBranch: GitHubRepositoryBranch = {...protectedMainBranch, protected: false,}

		const result = repository02([thePerfectRepo],[protectedMainBranch, unprotectedSideBranch]).map((repo) => repo.repository_02);
	...
	test('We should get a negative result when the default branch is not protected', () => {
		...
		const unprotectedMainBranch: GitHubRepositoryBranch = {
			repository_id: BigInt(1),
				name: 'default',
			protected: false,
			protection: {},
		...
		const result = repository02([repo],[unprotectedMainBranch]).map((repo) => repo.repository_02);
