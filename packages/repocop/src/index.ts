...
import { setRepoCustomProperty } from './remediation/branch-protector/github-requests.js';
...
export async function main() {
	...
	const [unarchivedRepos, _archivedRepos] = partition(
	...
	// temp function to add custom property to repos with topics
	...
	const productionAndDocumentationRepos = unarchivedRepos.filter(
		(repo) =>
			repo.topics.includes(PRODUCTION_VALUE) ||
			repo.topics.includes(DOCUMENTATION_VALUE),
	);
	console.log(
		`Found ${productionAndDocumentationRepos.length} unarchived repos with ${PRODUCTION_VALUE} and/or ${DOCUMENTATION_VALUE} topics`,
	);

	for (const repo of productionAndDocumentationRepos) {
		const propertyValue = getRepoPropertyValue(repo);

		// test with one repo without the custom property
		if (repo.name === 'PrivateBin') {
			...
			console.log(
				`Would have set ${config.gitHubOrg}/${repo.name}'s custom property ${PROPERTY_NAME} to ${propertyValue}`,
			);
		}
	}
}
