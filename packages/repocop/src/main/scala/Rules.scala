...
object Rules {
  ...
  enum RepoRule(
      statement: Repository => Boolean,
      val violationMessage: String,
      val ruleJustification: String = ""
  ):
  ...
  def evaluateReposForTeam(
      repos: List[Repository],
      teamSlug: String
  ): List[EvaluatedRepo] = {
    val teamRepos: List[Repository] = repos.filter { r =>
      r.rerunRepocop & r.owners.contains(teamSlug)
    }
  ...
  def evaluateRulesForRepo(repository: Repository): Map[String, Boolean] = {
    ...
    val ruleBools: List[Boolean] =
      Rules.RepoRule.values.map(_.evaluate(repository)).toList
  ...
  private def repoContainsProductionStatus(r: Repository): Boolean =
    r.topics.exists(isAllowedTopic)
  ...
  private def stripMarginAndInline(s: String): String =
