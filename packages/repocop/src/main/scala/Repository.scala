...
case class Repository(
  ...
  private val dateOfLastChange: Try[LocalDateTime] = Try(
    List(
  ...
  private val midnightYesterday: LocalDateTime =
    LocalDateTime.now().truncatedTo(ChronoUnit.DAYS).minusDays(1)

  val rerunRepocop: Boolean = dateOfLastChange match
    case Failure(
          e
        ) => // if we can't fetch the dates, rerun anyway unless repo has been archived
      println(s"Failed to parse timestamps for $name")
      !archived
    case Success(latestUpdate) =>
      latestUpdate.isAfter(midnightYesterday) & !archived
}
