...
1. Retrieve Deploy Tools credentials
2. Run the setup script (only if running for the first time, or for the first time in a while)
3. Create a local [CloudQuery](../dev-environment/README.md) database by running `npm run start -w dev-environment`, and wait for the tables to be populated
4. Run repocop using `npm -w repocop start`
